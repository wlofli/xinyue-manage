...
function changePage(page){
	...
	document.getElementById("searchForm").attributes['action'].value = "${ctx}/link/friendship/search?index="+index;
	$("#searchForm").submit();
}
</script>
</head>
...
<div class="c_right">
	<div class="c_r_bt">
		<h1><img src="${ctx}/images/lj_tb1.png" alt="友情链接" /><span>友情链接</span></h1>
		<c:if test="${authorities.friendship_add==1}">
			<a onclick="addOrUpdate('a')">添加友情链接</a>
		</c:if>
	</div>
	<div class="c_r_bt1">
		<sf:form action="${ctx}/link/friendship/search?index=${page.nowPage-1}" commandName="searchFriendShip" method="post" id="searchForm">
			<ul>
			<li><span>链接名称：</span><sf:input class="s1" path="linkName"/></li>
			<li><span>链接类型：</span>
			...
			<li>
				...
				<sf:select path="linkStatus" class="s2">
					...
					<sf:options items="${linkPublish}" itemLabel="value" itemValue="key"/>
				</sf:select>
			</li>
			<li><span>所属城市：</span>
			...
			<tbody>
				<c:forEach items="${fsList}" var="list" varStatus="vs">
					<tr>
						...
						<td colspan="1">
							<input type="text" value="${list.linkIndex}" class="t2" onchange="changeIndex(${vs.index+1},this.value)"/>
							<input type="hidden" value="${list.linkIndex}" id="hid_index${vs.index+1}">
						</td>
						<td colspan="1">
							<c:choose>
						...
						<td colspan="2">${list.linkText}</td>
						<td colspan="3">${list.linkUrl}</td>
						<td colspan="1">
						...
						<td colspan="2">
							...
							<c:if test="${authorities.friendship_delete==1}">
	...
	<div class="page">
		<ul class="btn">
			...
			<c:if test="${authorities.friendship_publish==1}">
...
function addOrUpdate(type,id){
	switch (type) {
	case "a":
		document.location.href = "${ctx}/link/friendship/addpage";
		break;
	case "u":
		document.location.href = "${ctx}/link/friendship/updatepage?code="+$("#hid_"+id).val();
	default:
		break;
	}
}
