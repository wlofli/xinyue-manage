...
<script>
	...
	function changePage(page){
		var index = page - 1;
		document.getElementById("searchForm").attributes['action'].value = "${ctx}/fastproduct/list/product?index="+index;
		$("#searchForm").submit();
...
<div class="c_r_bt1">
<sf:form action="${ctx }/fastproduct/list/product?index=0" commandName="fastproduct" method="post" id="searchForm">
<ul>
...
<table class="table2" cellpadding="0" cellspacing="0"><thead>
<tr>
<td colspan="2">序号</td>
<td colspan="2">订单号</td>
...
<td colspan="2">${list.organization }</td>
<td colspan="2">${list.credit }</td>
<td colspan="2"><fmt:formatDate value="${list.createdTime}" type="date"/></td>
<td colspan="2">${list.contactName }</td>
<td colspan="2">${list.contactPhone }</td>
<td colspan="3">${list.company }</td>
