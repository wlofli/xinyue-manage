		...
		<c:if test="${not empty answer.atime }">
			<c:forEach items="${answerpage.data }" var="quest" varStatus="vs">
				<div class="c_wtxq_nr">
					...
					<p class="nr">
						<span>回答者：<strong><c:if test="${quest.atype eq 'm' or quest.atype eq 'c' }" var="aflag">answerName</c:if><c:if test="${not aflag }">匿名</c:if></strong></span>
			...
			if($("#answer_form").valid()){
				$.ajax({
					...
					success:function(data){
						if(data == 'success'){
							alert("添加成功");
							document.location.href='${ctx}/member/quest?memberid=${memberid}';
						}else{
							alert("添加失败");
						}
					}
